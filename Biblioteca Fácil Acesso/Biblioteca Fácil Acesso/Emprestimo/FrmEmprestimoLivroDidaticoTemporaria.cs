    ...
    public partial class FrmEmprestimoLivroDidaticoTemporaria : Form
    {
        ...
        private void btn_EfetuaEmprestimo_Click(object sender, EventArgs e)
        {
            ...
            else if (num_Quantidade.Value == 0)
            {
                ...
                MessageBox.Show("Valor do campo quantidade inválida", "Biblioteca Fácil", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            ...
            if (rb_EfetuarEmprestimo.Checked)
            {
                ...
                try
                {
                    ...
                    MessageBox.Show("Empréstimo efetuado com sucesso!", "Biblioteca Fácil", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    cb_Professora.Focus();
                    Limpar();
                }
                catch (Exception ex)
            ...
            else if (rb_FazerDevolucao.Checked)
            {
                ...
                if (table.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        conexao.Open();
                        comando.ExecuteNonQuery();
                        MessageBox.Show("Devolução do(s) livro(s) efetuado com sucesso!", "Biblioteca Fácil", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Limpar();
                    }
                    catch (Exception ex)
                    {
                ...
                {
                    MessageBox.Show("Dados não compatíveis com os dados de entrada. Verifique os dados, principalmente a quantidade de livros emprestados!", "Biblioteca Fácil", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
